...
| `WAF_COUNTER_DATA` | `<профили>.applied`; в образе `/var/lib/waf/counter` | куда раскатка кладёт применённое поколение |
| `WAF_COUNTER_GEO_ADDR` | пусто | кодер гео (`host:port`); пусто — оси `asn_net` и `asn_router` молчат, остальные работают |
| `WAF_COUNTER_GEO_TIMEOUT` | `500ms` | ожидание кодера в бюджете сообщения |
| `WAF_COUNTER_GEO_NEG_MAX` | `0` | потолок отрицательного кэша клиента кодера; `0` — умолчание |
...
Проба идёт тем же путём, что боевая инспекция, и получает `deny` с пустой корзины — без Redis и без
...
- **Профиль `_probe` едет в образе, а не поколением.** Поколение его не везёт; процесс подмешивает
