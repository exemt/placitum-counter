 ...
 *
 * Обменник пробе не нужен намеренно. Профиль _probe держит правило judge с
 * порогом 0: пустая корзина уже на пороге, и ответ deny приходит без единого
 * заряда -- поэтому healthcheck работает и там, где Redis инспектору не дали,
 * и не зависит от того, читает ли маршрут тело.
 *
 *     counter-probe --uri /healthcheck
 *     counter-probe --profile default --uri /api/orders --expect allow
 */
...
func main() {
	var (
		...
		subject = flag.String("subject", env("WAF_COUNTER_SUBJECT", "waf.req.counter"),
			"subject инспектора")
		name = flag.String("inspector", env("WAF_COUNTER_NAME", "counter"),
