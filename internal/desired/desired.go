/*
 * Поколение профилей счётчика из JetStream KV.
 *
 * Манифест несёт файлы каталогов профилей; общая секция счётчиков едет тем же
 * манифестом псевдопрофилем _shared с единственным файлом counters.yaml --
 * формат манифеста этого не замечает, а загрузчик каталога читает _shared сам
 * и профилем его не считает. Поэтому здесь нет ни разбора профиля, ни валидации:
 ...
 * которого инспектор начнёт отказывать всем подряд, применять нельзя даже
 * частично.
 */
func Apply(store *config.Store, dataDir string, m *Manifest) error {
	...
	if err := store.ReloadFrom(live); err != nil {
		/*
		 * Профиль не разобрался или схема не скомпилировалась. Возвращаем
		 * прежнее дерево и перечитываем его:
...
func write(dir string, m *Manifest) error {
	...
	for name, profile := range m.Profiles {
		...
		for _, file := range profile.Files {
			// 0600: спецификация описывает внутренний API целиком, и читать
			// её посторонним незачем даже внутри контейнера.
