...
// PriorVerdict -- чужой вердикт, уже применённый модулем. Phase отличает
// вердикты предыдущих волн этой же фазы от вердиктов предыдущей фазы: на фазе
// ответа приезжают и те, и другие, а смысл у них разный.
type PriorVerdict struct {
	Phase     string `json:"phase"`
	Inspector string `json:"inspector"`
	Verdict   string `json:"verdict"`
	Score     int    `json:"score"`
	// Actions -- просьбы соседей. Значат что-то только через правило
	// trigger.prior профиля; без правила они видны лишь в аудите.
	Actions []Action `json:"actions"`
}

/*
 * Session -- личность, которую сосед узнал на этом запросе (секция sessions
 * сообщения модуля). Модуль её не толкует: он держит сказанное калиткой и
 * возвращает соседям, сквозной по фазам -- на фазе ответа приезжает то же,
 * что калитка сказала на запросе.
 *
 * User -- субъект (логин), ID -- идентификатор сессии: у своей сессии калитки
 * это sid, у чужой -- хеш. Verified -- проверял ли отправитель подпись; ключ
 * счёта берётся только с проверенных, иначе им управлял бы клиент.
 */
type Session struct {
	Inspector string   `json:"inspector"`
...
// Глаголы действий. Словарь общий для контура; из него этот инспектор
// применяет DoThreshold, DoSkip и DoNote -- остальные ему не адресованы, и
// загрузчик профиля отвергает правило с чужим глаголом.
const (
	...
	DoReauth    = "reauth"
	DoNote      = "note"
	// DoMutate -- переключить группы модификаторов у rewrite на этом
	// запросе или кадре: счётчик его только шлёт, сам не применяет.
	DoMutate = "mutate"
...
// Оси: о ком высказывание. threshold и skip ездят только с request; note --
// про субъекта: адрес, сеть, сессия.
const (
	ApplyRequest = "request"
	ApplyIP      = "ip"
	ApplyASN     = "asn"
	ApplySession = "session"
	// ApplyConn -- только у управляющих глаголов и только на кадрах: до
	// конца соединения, а не кадра.
	ApplyConn = "conn"
 ...
 * различались бы ровно этим полем и разъезжались бы при каждом расширении
 * словаря.
 */
type Action struct {
	...
	// Delta -- только при DoThreshold: проценты коэффициента к отдаваемому
	// счёту, множитель 1 + delta/100. Value -- только при DoNote: проценты
	// ёмкости корзины, которую называет правило приёма; знак значим, -100
	// обнуляет.
	Delta int `json:"delta,omitempty"`
	Value int `json:"value,omitempty"`
	// Counter -- только при DoNote: имя корзины получателя, селектор поверх
	// правила приёма. Пусто -- корзину называет правило; названная корзина
	// проходит только через правило, которое её и выдаёт: грант остаётся у
	// получателя.
	...
	// модификаторов получателя переключить и куда (on | off). Что именно --
	// называет отправитель, как корзину у note; дают ли ему это, решает
	// правило приёма получателя.
...
type Request struct {
	...
	// Sessions -- кого соседи узнали на этом запросе: источник ключа оси
	// user, когда он назван словом session. Пусто -- личности никто не
	// назвал: калитки на маршруте нет, она промолчала или она пассивна.
	...
	// соединения (ray рукопожатия), Seq -- номер кадра в нём; Stream --
	// кадрирование: протокол, направление, опкод, fin, подпротокол.
...
type Reply struct {
	...
	// Cache -- можно ли модулю повторить этот вердикт по хешу кадра
	// (waf_frame_cache). Счётчик судит по корзинам, а не по содержимому:
	// на кадрах он всегда отвечает false. Отсутствие поля -- «можно».
	Cache *bool `json:"cache,omitempty"`
}

// Bool -- указатель на литерал для необязательных булевых полей реплая.
