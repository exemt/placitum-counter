 ...
 * из своего буфера (COUNTER_DEADLINE_EXCEEDED), чтобы слот занял свежий запрос.
 * Полная очередь живых — drop (COUNTER_QUEUE_LIMIT) или wait до места / дедлайна.
...
const (
	ReasonQueueLimit       = "COUNTER_QUEUE_LIMIT"
	ReasonDeadlineExceeded = "COUNTER_DEADLINE_EXCEEDED"
...
type Task struct {
	...
	// Personal: сообщение пришло в личный subject экземпляра, а не в групповой.
	// Нужно только диагностике фазы ответа -- по самому сообщению этого не
